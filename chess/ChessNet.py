...
class ChessNetWrapper(NeuralNet):
    def __init__(self, game):
        ...
        self.net = ChessNNet(self.action_size).to(self.device)
    ...
    def train(self, examples):
        ...
        for epoch in range(10):
            ...
            for board, pi, v in examples:
                board = torch.tensor(board, dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(self.device)
                pi = torch.tensor(pi, dtype=torch.float32).to(self.device)
                v = torch.tensor([v], dtype=torch.float32).to(self.device)

                log_pi_pred, v_pred = self.net(board)

                loss_pi = -torch.sum(pi * log_pi_pred)
                loss_v = F.mse_loss(v_pred.squeeze(), v.squeeze())
                loss = loss_pi + loss_v
    ...
    def predict(self, board):
        self.net.eval()
        with torch.no_grad():
            x = torch.tensor(board, dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(self.device)
            log_pi, v = self.net(x)
